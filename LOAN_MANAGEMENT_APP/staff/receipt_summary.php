...
require_login();
require_roles(['CASHIER','ADMIN','MANAGER','CREDIT_INVESTIGATOR','LOAN_OFFICER']);

$customer_id = intval($_GET['customer_id'] ?? 0);
$customer = fetch_one(q("SELECT * FROM customers WHERE customer_id=?", "i", [$customer_id]));
...
$payments = fetch_all(q("SELECT p.*, l.reference_no, l.principal_amount, l.interest_rate, u.full_name AS cashier_name
                        ...
                        WHERE l.customer_id=?
                        ORDER BY p.payment_date DESC", "i", [$customer_id]));

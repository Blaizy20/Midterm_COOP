...
$loan = fetch_one(q("SELECT l.*, c.customer_no, CONCAT(c.first_name,' ',c.last_name) AS customer_name, u.full_name AS officer_name
                    FROM loans l 
...
if ($_SERVER['REQUEST_METHOD']==='POST') {
  ...
  if ($amount <= 0) $err='Invalid amount.';
    ...
    // Update loan officer if provided
    if ($officer_id > 0) {
      q("UPDATE loans SET loan_officer_id=? WHERE loan_id=?", "ii", [$officer_id, $loan_id]);
    }
    ...
    try {
    q("INSERT INTO payments (loan_id, amount, payment_date, method, cheque_number, cheque_date, bank_name, account_holder, bank_reference_no, gcash_reference_no, or_no, loan_officer_id, received_by, notes) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
        "idssssssssssis", [$loan_id, $amount, $date, $method, $cheque_number ?: null, $cheque_date ?: null, $bank_name ?: null, $account_holder ?: null, $bank_reference_no ?: null, $gcash_reference_no ?: null, $or, $officer_id, $_SESSION['user_id'], $notes]);
      $payment_id = db()->insert_id;
      log_activity('Payment Recorded', 'Payment of ₱' . number_format($amount, 2) . ' recorded via ' . $method . ' - OR#' . $or, $loan_id, $loan['customer_id'], $loan['reference_no']);
      
      // --- CRITICAL FIX: REMOVED THE BLOCK THAT AUTO-UPDATED DUE DATE ---
      // We do NOT want to move the due date automatically, otherwise
      // the Late Fee calculation will break on the next line.
      
      // Now recalc (This will keep the late fee because due date hasn't moved)
      recalc_loan($loan_id);
      
      // Redirect to loan view to show updated status
...
$loan_officers = fetch_all(q("SELECT user_id, full_name FROM users WHERE role='LOAN_OFFICER' AND is_active=1 ORDER BY full_name", ""));
...
<div class="card">
  ...
  <div class="small">Loan: <b><?= htmlspecialchars($loan['reference_no']) ?></b> • Customer: <?= htmlspecialchars($loan['customer_name']) ?> (<?= htmlspecialchars($loan['customer_no']) ?>)</div>
  ...
  <div style="display:grid;grid-template-columns:repeat(5,1fr);gap:16px;margin-top:20px;margin-bottom:20px">
    <div class="card">
      ...
      <div style="font-size:26px;font-weight:800;color:#000000"><?= $loan['principal_amount']===null?'—':'₱' . number_format($loan['principal_amount'], 2) ?></div>
    </div>
    
    <div class="card">
      <div class="small" style="color:var(--muted);font-weight:600;margin-bottom:8px">TOTAL PAYABLE</div>
      <div style="font-size:26px;font-weight:800;color:#000000"><?= $loan['total_payable']===null?'—':'₱' . number_format($loan['total_payable'], 2) ?></div>
    </div>
    
    <div class="card">
      <div class="small" style="color:var(--muted);font-weight:600;margin-bottom:8px">REMAINING BALANCE</div>
      <div style="font-size:26px;font-weight:800;color:#000000" id="remaining"><?= $loan['remaining_balance']===null?'—':'₱' . number_format($loan['remaining_balance'], 2) ?></div>
    </div>
    ...
    <div class="card">
      ...
      <div style="font-size:26px;font-weight:800;color:#000000"><?php 
        ...
        echo isset($terms[$loan['payment_term']]) ? htmlspecialchars($terms[$loan['payment_term']]) : (htmlspecialchars($loan['payment_term']) ?: '—');
      ?></div>
    </div>
    ...
    <div class="card">
      ...
      <div style="font-size:26px;font-weight:800;color:#000000"><?php 
        ...
        echo $rate ? htmlspecialchars($rate) . '%' : '—';
        ?></div>
    </div>
  </div>
...
document.getElementById('amount').addEventListener('input', (e)=>{
  ...
  document.getElementById('after').textContent = "Remaining after: ₱" + parseFloat(after).toLocaleString('en-US', {minimumFractionDigits:2, maximumFractionDigits:2});
});

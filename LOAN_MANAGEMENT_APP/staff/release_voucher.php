...
$existing = fetch_one(q("SELECT * FROM money_release_vouchers WHERE loan_id=? AND status!='CANCELLED' LIMIT 1", "i", [$id]));
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_voucher'])) {
    ...
    if ($existing) {
        q("UPDATE money_release_vouchers SET release_date=?, check_amount=?, received_by_name=?, voucher_data=?, status='DRAFT' WHERE loan_id=?",
          "sdssi", [$post_date, $final_release_amount, $post_received, $v_data, $id]);
    } else {
        q("INSERT INTO money_release_vouchers (loan_id, voucher_no, release_date, check_no, check_amount, prepared_by, received_by_name, voucher_data) VALUES (?,?,?,?,?,?,?,?)",
          "isssdsss", [$id, $voucher_no_auto, $post_date, 'MANUAL', $final_release_amount, $_SESSION['user_id'], $post_received, $v_data]);
    }

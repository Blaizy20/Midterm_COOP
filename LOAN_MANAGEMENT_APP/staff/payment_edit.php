...
$p = fetch_one(q("SELECT p.*, l.reference_no FROM payments p JOIN loans l ON l.loan_id=p.loan_id WHERE p.payment_id=?", "i", [$id]));
...
if ($_SERVER['REQUEST_METHOD']==='POST' && isset($_POST['save_payment']) && $otp_verified) {
  ...
  if ($amount <= 0) $err='Invalid amount.';
    ...
    $loan = fetch_one(q("SELECT customer_id, reference_no FROM loans WHERE loan_id=?", "i", [$p['loan_id']]));
    if ($loan) {

...
$reqs = fetch_all(q("SELECT * FROM requirements WHERE loan_id=? ORDER BY uploaded_at DESC", "i", [$id]));
...
$payments = fetch_all(q("SELECT p.*, u.full_name AS cashier_name FROM payments p LEFT JOIN users u ON u.user_id=p.received_by WHERE p.loan_id=?$payment_where ORDER BY p.payment_date DESC, p.payment_id DESC", $payment_types, $payment_params));

$err=''; $ok='';

// CI action: mark reviewed
if ($_SERVER['REQUEST_METHOD']==='POST' && isset($_POST['ci_review'])) {
  require_roles(['CREDIT_INVESTIGATOR','ADMIN']);
...
if ($_SERVER['REQUEST_METHOD']==='POST' && isset($_POST['update_terms'])) {
  ...
  if ($update_type === 'interest_only') {
    if ($rate <= 0) {
      ...
      // Refetch
      $loan = fetch_one(q("SELECT l.*, c.customer_no, CONCAT(c.first_name,' ',c.last_name) AS customer_name, c.contact_no, u.full_name AS officer_name FROM loans l JOIN customers c ON c.customer_id=l.customer_id LEFT JOIN users u ON u.user_id=l.loan_officer_id WHERE l.loan_id=?", "i", [$id]));
    }
  }
...
if ($_SERVER['REQUEST_METHOD']==='POST' && isset($_POST['assign_officer'])) {
  ...
  if ($officer_id <= 0) $err='Please select a loan officer.';
    ...
    $officer = fetch_one(q("SELECT user_id, full_name FROM users WHERE user_id=? AND is_active=1", "i", [$officer_id]));
...
<div class="card">
    ...
    <div class="col">
      <div class="card" style="background:#fff">
            ...
            <?php foreach($payments as $p): ?>
              <tr>
                ...
                <td style="display:flex;gap:6px;flex-wrap:wrap">
                  ...
                  <?php endif; ?>
                  <?php if (in_array($_SESSION['role'] ?? '', ['CASHIER','ADMIN'], true)): ?>
                    <a class="btn btn-primary" href="<?php echo APP_BASE; ?>/staff/payment_edit.php?id=<?= intval($p['payment_id']) ?>" style="font-size:12px">Edit</a>
                  <?php endif; ?>
                  <?php if (empty($_SESSION['role']) || !in_array($_SESSION['role'] ?? '', ['CASHIER','ADMIN','MANAGER'], true)): ?>—<?php endif; ?>
                </td>
              </tr>
            <?php endforeach; ?>
        ...
        <div style="margin-top:12px;display:flex;justify-content:flex-end">
          <?php if (in_array($_SESSION['role'] ?? '', ['CASHIER','ADMIN','MANAGER'], true) && in_array($loan['status'], ['ACTIVE','OVERDUE'], true)): ?>
  ...
  <?php if (in_array($loan['status'], ['PENDING'], true) && in_array($_SESSION['role'] ?? '', ['CREDIT_INVESTIGATOR','ADMIN'], true)): ?>
    <div class="card" style="background:#fff">
  ...
  <?php if (in_array($loan['status'], ['PENDING','CI_REVIEWED'], true) && in_array($_SESSION['role'] ?? '', ['MANAGER','ADMIN'], true)): ?>
    <div style="height:14px"></div>
    <div class="card" style="background:#fff">
  ...
  <?php if (in_array($loan['status'], ['ACTIVE','OVERDUE','APPROVED'], true) && in_array($_SESSION['role'] ?? '', ['MANAGER','ADMIN'], true)): ?>
    <div style="height:14px"></div>
    <div class="card" style="background:#fff">
    ...
    <div class="card" style="background:#fff">
      ...
      <form method="post">
        ...
        <select class="input" name="loan_officer_id">
          ...
          <?php
            $staff = fetch_all(q("SELECT user_id, full_name FROM users WHERE role='LOAN_OFFICER' AND is_active=1 ORDER BY full_name", ""));
            foreach ($staff as $s) {

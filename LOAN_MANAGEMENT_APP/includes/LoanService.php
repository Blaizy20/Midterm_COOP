...
class LoanService {
    ...
    public function getLoanById($loanId) {
        try {
            ...
            return $this->db->queryOne($sql, [$loanId]);
        } catch (Exception $e) {
    ...
    public function getLoanByReferenceNo($refNo) {
        try {
            ...
            return $this->db->queryOne($sql, [$refNo]);
        } catch (Exception $e) {
    ...
    public function canCustomerApplyLoan($customerId) {
        try {
            ...
            $unpaidLoans = $this->db->queryAll($sql, [$customerId]);
    ...
    public function createLoan($customerId, $data) {
        try {
            ...
            $sql = "INSERT INTO loans 
                    (reference_no, customer_id, principal_amount, interest_rate, payment_term, 
    ...
    public function updateLoanStatus($loanId, $newStatus, $userId = null, $notes = null) {
        try {
            ...
            if ($newStatus === 'CI_REVIEWED' && $userId) {
                $sql = "UPDATE loans SET status = ?, ci_by = ?, ci_at = NOW() WHERE loan_id = ?";
                $this->db->exec($sql, [$newStatus, $userId, $loanId]);
            } elseif ($newStatus === 'APPROVED' && $userId) {
                $sql = "UPDATE loans SET status = ?, manager_by = ?, manager_at = NOW() WHERE loan_id = ?";
                $this->db->exec($sql, [$newStatus, $userId, $loanId]);
            } elseif ($newStatus === 'ACTIVE' && $userId) {
                $sql = "UPDATE loans SET status = ?, loan_officer_id = ?, activated_at = NOW(), 
                        due_date = DATE_ADD(NOW(), INTERVAL ? DAY) WHERE loan_id = ?";
                $termMonths = $loan['term_months'] ?? 12;
                $termDays = $termMonths * 30; // Approximate
                $this->db->exec($sql, [$newStatus, $userId, $termDays, $loanId]);
            } else {
                $sql = "UPDATE loans SET status = ? WHERE loan_id = ?";
                $this->db->exec($sql, [$newStatus, $loanId]);
            }
            
            // Add note if provided
            if ($notes) {
                $sql = "UPDATE loans SET notes = CONCAT_WS('\\n', IFNULL(notes, ''), ?) WHERE loan_id = ?";
                $this->db->exec($sql, [$notes, $loanId]);
            }
    ...
    public function getLoansByStatusQueue($status) {
        try {
            ...
            return $this->db->queryAll($sql, [$status]);
        } catch (Exception $e) {
    ...
    public function countLoansByStatus($status) {
        try {
            $sql = "SELECT COUNT(*) as count FROM loans WHERE status = ?";
